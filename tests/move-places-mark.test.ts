...
//
// This unit test proves the pure rule behind that UI behaviour:
//   move(board, index, player) places `player`'s mark at an empty `index` and
//   returns a NEW board, leaving the ORIGINAL board unmutated (immutability).
//
// The module under test (games/tictactoe/game.js) is a UMD/CommonJS file with no
// type declarations, so it is loaded via require. The spec's `app` import root is a
// Python convention that does not apply to this in-repo relative JS module; there is
// no tsconfig `paths` / package `imports` alias mapping a bare `app/...` specifier.
...
describe("move(board, index, player) — places mark on a new board (AC#2)", () => {
  ...
  it("only sets the target cell, leaving every other cell empty", () => {
    const board = emptyBoard();
