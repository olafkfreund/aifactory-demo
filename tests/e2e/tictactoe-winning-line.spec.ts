...
//
// Subtask: winning-line-visibly-marked-ui — Verify the UI highlights the three
// winning cells (`.win` class) once a line completes.
//
// This Playwright test drives games/tictactoe/index.html (::render) in a real
// browser over the file:// protocol — no server, no build. For every one of the
// 8 winning lines (3 rows, 3 columns, 2 diagonals) it plays a deterministic
// winning sequence and proves that:
//   * the status region announces the winner,
//   * EXACTLY 3 cells carry the `.cell.win` highlight class,
//   * those 3 highlighted cells are precisely the winning line's indices — no
//     off-line cell is decorated and no winning cell is missed.
// It also confirms that no cell is highlighted while play is still in progress,
// so the count of 3 is a property of the completed line rather than always-on
// decoration.
...
// All 8 winning lines AC#4 enumerates. Each case's `moves` is the click order
// (X, O, X, O, X) that lets X complete `line` while O plays two harmless
// off-line cells, so exactly X wins on the named line.
...
test.describe('tic-tac-toe completed winning line is visibly marked on all 8 lines (AC#4)', () => {
  for (const { name, line, moves } of WIN_CASES) {
    test(`completing the ${name} marks exactly the 3 winning cells`, async ({
      page,
    }) => {
      ...
      // Exactly 3 cells — the completed line — carry the `.win` highlight, and
  ...
  test('no cell is marked while the game is still in progress', async ({
    page,
  }) => {
    await page.goto(INDEX_URL);
