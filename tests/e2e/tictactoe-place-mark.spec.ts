...
// turn.
//
// Target: games/tictactoe/index.html::board — each cell is a role=gridcell
// button whose click handler calls TicTacToe.move(board, i, currentPlayer);
// on a real (non-no-op) move it flips currentPlayer and re-renders, so the
// clicked cell shows the acting player's mark and the role=status region
// advances to the other player's turn ("X's turn" -> "O's turn").
//
// This test loads the page directly over file:// (no server, no build step),
// clicks empty cells, and asserts each click renders the acting player's mark
// and passes the turn.
...
// specs/187-verify-tictactoe-lanes/tests/e2e, and the game under test ships
// inside the sibling .worktree copy of the repo, so probe the known relative
...
test.describe('tic-tac-toe: clicking an empty cell places a mark and passes the turn (AC#2)', () => {
  ...
  test('clicking an empty cell shows X and advances the status to O', async ({ page }) => {
    ...
    // Click the empty center cell: the current player's (X's) mark appears.
  ...
  test('a follow-up click places O and passes the turn back to X', async ({ page }) => {
    const cells = page.getByRole('gridcell');
    const status = page.getByRole('status');
  ...
  test('a single move marks only the clicked cell', async ({ page }) => {
    const cells = page.getByRole('gridcell');
