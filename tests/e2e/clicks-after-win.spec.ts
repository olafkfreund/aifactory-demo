...
//
// This E2E test drives games/tictactoe/index.html (the #board render() flow) in
// a real browser. It plays a deterministic winning sequence so X wins on the top
// row, then clicks cells that are still empty after the game is over. It proves
// nothing changes: clicked cells stay empty (no mark placed, no turn passed), the
// status keeps announcing the winner, and exactly the three winning cells stay
// highlighted.
...
test.describe('tic-tac-toe: clicks after a win are ignored (AC#6)', () => {
  ...
  test('clicking an empty cell after a win changes neither the board nor the status', async ({
    page,
  }) => {
    ...
    await expect(cells.nth(8)).toHaveText('');
    // Winner status is unchanged (not "O's turn", not a draw).
    await expect(page.getByRole('status')).toHaveText('X wins!');
  ...
  test('hammering every empty cell after a win never advances the game', async ({
    page,
  }) => {
    const cells = page.getByRole('gridcell');
    ...
    await expect(page.getByRole('status')).toHaveText('X wins!');
    // Still exactly the three original winning cells highlighted.
