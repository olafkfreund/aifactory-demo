...
//
// This E2E test drives games/tictactoe/index.html::board (the render() flow) in
// a real browser. It plays a deterministic winning sequence so X wins, then
// clicks cells that remain open after the win. It proves the UI ignores those
// clicks: no mark is placed, no turn passes, the status keeps announcing the
// winner, and exactly the three winning cells stay highlighted.
...
test.describe('tic-tac-toe UI ignores clicks after game over (AC#6)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(INDEX_URL);
  });

  test('clicking an empty cell after a win does not change the board or status', async ({
    page,
  }) => {
