...
// deterministic winning sequence (X completes the top row), then clicks a cell
// that is still empty after the game is over. It proves the board and the
// winner status are unchanged: the clicked cell stays empty, the status still
...
test.describe('tic-tac-toe stops accepting clicks after game over (AC#6)', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto(INDEX_URL);
  });

  test('clicking a remaining empty cell after a win leaves the board and status unchanged', async ({
    page,
  }) => {
    ...
    // Cell 6 is still empty and was never part of the winning line.
    await expect(cells.nth(6)).toHaveText('');

    // Click the remaining empty cell after game over — it must do nothing.
    await cells.nth(6).click();

    // The cell stays empty: no mark placed, no turn passed.
    await expect(cells.nth(6)).toHaveText('');
    // Winner status is unchanged (not "O's turn" or anything else).
    await expect(page.getByRole('status')).toHaveText('X wins!');
  ...
  test('multiple clicks on empty cells after a win never advance the game', async ({
    page,
  }) => {
    const cells = page.getByRole('gridcell');
