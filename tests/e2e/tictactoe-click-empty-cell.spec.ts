...
// turn.
//
// Target: games/tictactoe/index.html::render — each cell is a gridcell button
// whose click handler calls TicTacToe.move(board, i, currentPlayer). On a real
// (non-no-op) move the handler flips currentPlayer and calls render(), which
// writes the acting player's mark into the clicked cell and advances the
// #status region to the other player's turn. This test loads the page directly
// over file:// (no server, no build step), clicks empty cells through the UI,
...
test.describe("tic-tac-toe: clicking an empty cell marks it and passes the turn (AC#2)", () => {
  ...
  test("clicking an empty cell renders X and passes the turn to O", async ({ page }) => {
  ...
  test("a subsequent click places O and passes the turn back to X", async ({ page }) => {
    ...
    // The first mark is untouched — the second move did not disturb the board.
    await expect(cells.nth(0)).toHaveText("X");
  });
