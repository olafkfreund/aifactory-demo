...
// turn.
//
// Target: games/tictactoe/index.html::cell — each board cell is a
// role="gridcell" button whose click handler calls
// TicTacToe.move(board, i, currentPlayer). On a real (non-no-op) move the UI
// re-renders so the clicked cell shows the acting player's mark, flips
// currentPlayer, and advances the aria-live status to the other player's turn.
//
...
// under specs/<spec>/tests/e2e; the game under test ships under the sibling
// .worktree copy (and, in other layouts, as a plain games/tictactoe tree), so
...
test.describe('tic-tac-toe click empty cell marks and swaps turn (AC#2)', () => {
  ...
  test('a single move marks exactly one cell and leaves the rest empty', async ({ page }) => {
    ...
    // A single click must place exactly one mark and not leak into other cells.
